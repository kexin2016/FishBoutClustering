...
% function [boutAngle,eyeConv,whichFish,whichBout,deltaRadius,deltaAngle] = getPrepData(filepath,fishname)
    ...
    species = [];
    exptype = [];
    if ~exist(fullfile(filepath,'tail_swing.mat'),'file') || ~exist(fullfile(filepath,'high_analysis.mat'),'file')
        return;
    end
    load(fullfile(filepath,'tail_swing.mat'),'sum_curv','wrong_tail','curvdata','angle_data');
    ...
    elseif len_short>len_long
        [angle_data,sum_curv,wrong_tail,curvdata] = samfnmultvar(@(x) x(1:len_long,:),angle_data,sum_curv,wrong_tail,curvdata);
    end
    %%
...
%     end
    ...
    sum_curv(abs(sum_curv)>1) = sign(sum_curv(abs(sum_curv)>1))*1;
...
%     for i=1:length(boutAngle)
%         if(size(boutAngle{i},1)>30)
%             boutAngle{i}=boutAngle{i}(1:30,:);
%             eyeConv{i}=eyeConv{i}(1:30);
%         end
%     end
