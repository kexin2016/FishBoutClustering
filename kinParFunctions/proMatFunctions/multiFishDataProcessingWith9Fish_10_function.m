...
function [allFishStructure,FileInfStructure] = multiFishDataProcessingWith9Fish_10_function(a,fishUniqueNumber,uniqueFileNumber,realFishUniqueNumber,trueUniqueFishNumber,dataSetName,protocolNumber,date,experimentNumber,fishAge,frameRate,strain,numbFishInFile,numbColumns,experimentRepeat)
...
%         [stim,lagNumber,tailAngles,tailValues,bodyAngles,xPos,yPos,blobVal,pixelSize,headToBladderLength,tailSegmentLength,distanceEyesToBlob,dataSetNumber,arenaX,arenaY,protocolNumb,leftEyeAngle,rightEyeAngle,numbFishInSquare,parameciaCounter,frameNumber2] = DataBaseDetector_23(aThisFish,dataSetName);
 [stim,lagNumber,tailAngles,tailValues,bodyAngles,xPos,yPos,blobVal,pixelSize,headToBladderLength,tailSegmentLength,distanceEyesToBlob,dataSetNumber,arenaX,arenaY,protocolNumb,leftEyeAngle,rightEyeAngle,numbFishInSquare,parameciaCounter,frameNumber2] = DataBaseDetector_Joao(aThisFish,dataSetName);

 if isempty(stim)
 [stim,lagNumber,tailAngles,tailValues,bodyAngles,xPos,yPos,blobVal,pixelSize,headToBladderLength,tailSegmentLength,distanceEyesToBlob,dataSetNumber,arenaX,arenaY,protocolNumb,leftEyeAngle,rightEyeAngle,numbFishInSquare,parameciaCounter,frameNumber2] = DataBaseDetector_Simone(aThisFish,dataSetName);
 end
 
 if isempty(stim)
 [stim,lagNumber,tailAngles,tailValues,bodyAngles,xPos,yPos,blobVal,pixelSize,headToBladderLength,tailSegmentLength,distanceEyesToBlob,dataSetNumber,arenaX,arenaY,protocolNumb,leftEyeAngle,rightEyeAngle,numbFishInSquare,parameciaCounter,frameNumber2] = DataBaseDetector_Antonia(aThisFish,dataSetName);
 end
 
if isempty(stim)
 [stim,lagNumber,tailAngles,tailValues,bodyAngles,xPos,yPos,blobVal,pixelSize,headToBladderLength,tailSegmentLength,distanceEyesToBlob,dataSetNumber,arenaX,arenaY,protocolNumb,leftEyeAngle,rightEyeAngle,numbFishInSquare,parameciaCounter,frameNumber2] = DataBaseDetector_RitaFelix(aThisFish,dataSetName);
 end
  
 
 
 if isempty(stim)
    disp('Data set name is not in any of the lists!!!') 
...
%        [indNoCollisionStartAll,indNoCollisionEndAll,indFishInSquareStartAll,indFishInSquareEndAll,noCollision,fishInSquare] = ...
%            CollisionDetector_3(aThisFish,numbFishInSquare);
%        
%        
%        [collisionflag] = collisionFlagCreator_1(numbFishInFile,numbColumns,a);
       
       
indNoCollisionStartAll = NaN;
indNoCollisionEndAll = NaN; 
indFishInSquareStartAll = NaN; 
indFishInSquareEndAll = NaN; 

 noCollision =  size(aThisFish,1);

fishInSquare = zeros(1,size(aThisFish,1));
collisionflag = zeros(1,size(aThisFish,1));
