...
tic
parfor nn = (n+1) : length(indBigClusters)
% nn =1
...
%make histograms
% dim1HistCluster1 = hist(cluster1(:,1),-40:0.5:40);
% dim1HistCluster2 = hist(cluster2(:,1),-40:0.5:40);
% 
% dim2HistCluster1 = hist(cluster1(:,2),-40:0.5:40);
% dim2HistCluster2 = hist(cluster2(:,2),-40:0.5:40);
% 
% dim3HistCluster1 = hist(cluster1(:,3),-40:0.5:40);
% dim3HistCluster2 = hist(cluster2(:,3),-40:0.5:40);



%calculate ROC
% bothClusteraTogetherProj = [cluster1(:,1)' cluster2(:,1)']';
% bothClusteraTogetherProj = [cluster1(:,2)' cluster2(:,2)']';
% bothClusteraTogetherProj = [cluster1(:,3)' cluster2(:,3)']';
bothClusteraTogetherProj = [clusterProj1' clusterPorj2']';

indsClusters = [(zeros(1,length(cluster1))-1) (zeros(1,length(cluster2))+1)];


% [ROCareaDim1,~,~,~,~,~,~,~,~] = roc_sv(bothClusteraTogetherDim1,indsClusters,'nofigure');
% [ROCareaDim2,~,~,~,~,~,~,~,~] = roc_sv(bothClusteraTogetherDim2,indsClusters,'nofigure');
% [ROCareaDim3,~,~,~,~,~,~,~,~] = roc_sv(bothClusteraTogetherDim3,indsClusters,'nofigure');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ROCareaNormDimProjtest([51 78],:) = [];
% ROCareaNormDimProjtest(:,[51 78]) = [];
% indNaNs = find(isnan(ROCareaNormDimProjtest));
% ROCareaNormDimProjtest2 = ROCareaNormDimProjtest;
% 
% ROCareaNormDimProjtest2(indNaNs) = 0.7;
 cutThreshold = 16;


 Z1 = linkage(ROCareaNormDimProjtestNoNans,'average','seuclidean');
%     Z1 = linkage(ROCareaNormDimProjtestNoNans,'single','seuclidean');
...
for nn = 1:length(uniqueT2)
    
     inds = find(T2 == uniqueT2(nn));
%  inds = indAll(nn).inds;
...
clusterStructure(indsInclusterStructure(n)).ClusterOfClusterCat =  uniqueT2(nn);
...
thisClusterOriginalInds = clusterStructure(indsInclusterStructure(n)).thisClusterOriginalInds;
     ...
     hold on
         axis([-17 22 -10 20])     
axis square
title(length(indsInclusterStructure))
end
end


%%
% %%%%%%%%%%%%%%%% determie where gourp of clusters como from %%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
% for nn = 1:length(uniqueT2)
% %     nn =9
%      inds = find(T2 == uniqueT2(nn))
% %      inds = [77 207 237 221 226 263 165]
%      indsInclusterStructure = indBigClusters(inds);
%      
%      bigDataSetNumberThis = [clusterStructure(indsInclusterStructure).bigDataSetNumber];
%      
%      
%      for nnn = 1 : length(bigDataSetNumberThis)
%      dataSetThis = indStructure(bigDataSetNumberThis(nnn)).dataSet
% %      stimNumbThis =indStructure(bigDataSetNumberThis(nnn)).stimNumb
%      end
%       pause
%      
% end
...
% % % disp('done')
% % % pause
%        save('ROC_kNN4_74Kins3dims_1.75Smooth_fast_3000_auto.mat','BoutKinematicParameters','BoutInf','boutDataPCASample','behavioralSpaceStructure','indStructure','clusterStructure','ROCareaNormDimProjtest','ROCareaNormDimProjtestNoNans','-v7.3')
% % % ROC_AllData_74Kins3dims_1.75Smooth_slow_3000_square_100thres_1.mat
...
%%%%%%%%%%%%%%%%% save essemble t-SNE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% save('GroupsClusters_MK801_74Kins3dim_slow_3000_auto_NoControl.mat','BoutKinematicParameters','BoutInf','boutDataPCASample','behavioralSpaceStructure','indStructure','ROCareaNormDimProjtest','ROCareaNormDimProjtestNoNans','groupClusterSctructure','clusterStructure','-v7.3')
