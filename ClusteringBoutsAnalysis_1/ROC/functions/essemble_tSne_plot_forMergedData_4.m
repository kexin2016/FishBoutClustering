...
function [T2,Z2,essembleMatrixPerm,clusterStructure] = essemble_tSne_plot_forMergedData_4(essembleMatrix,clusterStructure,groupClusterSctructureThis,cutThreshold,numbDist,boutDataPCASample,makeplot,axisLim,rectSize)
...
% boutDataPCASampleDataThisClusterAll = [];
...
% boutDataPCASampleDataThisClusterThis = clusterStructure(v).boutDataPCASampleDataThisCluster;

% boutDataPCASampleDataThisClusterAll = vertcat(boutDataPCASampleDataThisClusterAll,boutDataPCASampleDataThisClusterThis);
...
rectPos = [axisLim(1) (axisLim(4)-rectSize) rectSize rectSize]; 
...
%     indThisGroupClusters = intersect(indCoreGroupClusters,indThisCluster);
    ...
    col2 = jet(length(indThisGroupClustersOriginal));
    
    if makeplot == 1
        
        if uniqueClusterAssignmentGroupClusters <= 16
        
        subplot(4,4,h)
    
        else
            
        subplot(ceil(sqrt(length(uniqueClusterAssignmentGroupClusters))),ceil(sqrt(length(uniqueClusterAssignmentGroupClusters))),h) 
        
        end
        
%     plot(boutDataPCASampleDataThisClusterAll(:,1),boutDataPCASampleDataThisClusterAll(:,2), '.k')
        plot(boutDataPCASample(indAllDataPoints,1),boutDataPCASample(indAllDataPoints,2), '.k')

    hold on
    rectangle('Position',rectPos,'FaceColor',col(h,:))
...
%     axis square
   ...
   indThisCluster =  clusterStructure(indThisGroupClustersOriginal(hh)).indsCore;

        
    clusterStructure(indThisGroupClustersOriginal(hh)).ClusterOfClusterCat = uniqueClusterAssignmentGroupClusters(h);

        
%     boutDataPCASampleDataThisCluster = clusterStructure(indThisGroupClustersOriginal(hh)).boutDataPCASampleDataThisCluster;
    
    if makeplot == 1
    plot(boutDataPCASample(indThisCluster,1),boutDataPCASample(indThisCluster,2), '.', 'color', col2(hh,:))
    hold on
    end
