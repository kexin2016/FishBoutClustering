function [clusterStructure] = makeClusterStructure_3(indStructure,minNumbClusterPoints,percClusterThres,corePerc,indToAddAllClusters,makeplot) 
...
%         indChoosenClusterCentersOrigianlInds = indChoosenClusterCenters(n);
       ...
       indsCore2 = 1 : length(thisClusterOriginalInds);
       ...
       indChoosenClusterCentersInThisCore = find(clusterCenterVector(indsCore2) == 1);
        ...
        %chck is this dataset size rule should be not used
        TF = 0;
        for j = 1 : length(indToAddAllClusters)
            
            if nn == indToAddAllClusters(j)
...
%     plot3(boutDataPCASampleDataThisCluster(:,1),boutDataPCASampleDataThisCluster(:,2),boutDataPCASampleDataThisCluster(:,3), '.', 'color','k')
    plot(boutDataPCASampleDataThisCluster(:,1),boutDataPCASampleDataThisCluster(:,2), '.', 'color','k')
    hold on
