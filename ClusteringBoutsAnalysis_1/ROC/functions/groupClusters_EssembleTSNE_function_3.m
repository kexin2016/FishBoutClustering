 ...
 function [groupClusterSctructureThis,essembleMatrix,essembleMatrixRand] = groupClusters_EssembleTSNE_function_4(ROCareaNormDimProjtestNoNans,numbOfTimes,numbDims,perplexity,densityType,scallingFactorGroupClusters,linedensityMethod,distanceTsne)
...
groupClusterSctructureThis = [];
 ...
 essembleMatrix = zeros(length(ROCareaNormDimProjtestNoNans),length(ROCareaNormDimProjtestNoNans));
 essembleMatrixRand = zeros(length(ROCareaNormDimProjtestNoNans),length(ROCareaNormDimProjtestNoNans));
...
mappedX = tsne_d(ROCareaNormDimProjtestNoNans, [], numbDims, perplexity);

else
    
mappedX = tsne(ROCareaNormDimProjtestNoNans, [], numbDims, perplexity);
