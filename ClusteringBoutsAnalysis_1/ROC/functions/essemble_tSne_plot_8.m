  ...
  function [T2,Z2,essembleMatrixPerm,clusterStructure] = essemble_tSne_plot_8(essembleMatrix,boutDataPCASample,clusterStructure,groupClusterSctructureThis,cutThreshold,numbDist,makeplot,axisLim,col)
...
if isempty(col) || size(col,1) ~= length(uniqueClusterAssignmentGroupClusters)
...
%     axis square
    ...
    if makeplot == 1
    plot(boutDataPCASample(thisClusterOriginalInds,1),boutDataPCASample(thisClusterOriginalInds,2), '.', 'color', col2(hh,:))
    hold on
    end
