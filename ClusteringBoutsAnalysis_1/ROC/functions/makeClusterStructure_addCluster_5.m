function [clusterStructure] = makeClusterStructure_addCluster_5(indStructure,boutDataPCASample,minNumbClusterPoints,percClusterThres,corePerc,indToAddAllClusters,indExcludeClusters,makeplot) 
...
%     boutDataPCASampleData = boutDataPCASample(thisIndRand,:);
...
%         clusterStructure(counter).boutDataPCASampleDataThisCluster = boutDataPCASampleData(indThisCluster,1:numbOfDims);
        ...
        %check if cluster should be added
        TF = 0;
        ...
        if TF == 1
        clusterStructure(counter).addCluster =  1;

        else
        clusterStructure(counter).addCluster =  0;
 
        end
        
        %check if cluster should be excluded
        TF2 = 0;
        for j = 1 : length(indExcludeClusters)
            
            if counter == indExcludeClusters(j)
        ...
        if TF2 == 1
        clusterStructure(counter).excludeCluster =  1;

        else
        clusterStructure(counter).excludeCluster =  0;
...
n
    ...
    addCluster = clusterStructure(n).addCluster;
    excludeCluster = clusterStructure(n).excludeCluster;
...
%     plot3(boutDataPCASampleDataThisCluster(:,1),boutDataPCASampleDataThisCluster(:,2),boutDataPCASampleDataThisCluster(:,3), '.', 'color','k')
    plot3(boutDataPCASample(thisClusterOriginalInds,1),boutDataPCASample(thisClusterOriginalInds,2),boutDataPCASample(thisClusterOriginalInds,4), '.', 'color','k')
    hold on
