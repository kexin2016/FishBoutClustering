...
indThisBoutType1 = intersect(indGoodBouts,find(boutCat == uniqueBoutCat(n)));
tic
...
if nn ~= n
    ...
    indThisBoutType2 = intersect(indGoodBouts,find(boutCat == uniqueBoutCat(nn)));
...
%     thisKinOtherBoutType = BoutKinematicParametersFinalArray(indOtherBoutTypes,indKinParsFinal(nn));
    ...
    indRand1 = inds1(indPerm1(1:numBouts));
    
    
    inds2 = 1 : length(thisKinThisBoutType2);
    indPerm2 = randperm(length(inds2));
    indRand2 = inds2(indPerm2(1:numBouts));

thisKinThisBoutType1Rand = thisKinThisBoutType1(indRand1);
thisKinThisBoutType2Rand = thisKinThisBoutType2(indRand2);

% subplot(1,2,1)
%    [] = hist(thisKinThisBoutType1Rand,5)
% hold on
% subplot(1,2,2)
% 
%     hist(thisKinThisBoutType2Rand,5)


     indNaNs1 =  find(isnan(thisKinThisBoutType1Rand));
     indNaNs2 =  find(isnan(thisKinThisBoutType2Rand));
     

   percNaNs1 = length(indNaNs1)/length(thisKinThisBoutType1Rand);
   percNaNs2 = length(indNaNs2)/length(thisKinThisBoutType2Rand);

   percNaNsBoth = min([percNaNs1 percNaNs2]);
    
    if  percNaNsBoth < 0.05%exclude kin pars that have many mistakes
        
    dataTogether = [hisKinThisBoutType1Rand' thisKinThisBoutType2Rand']';

    indsDist = [(zeros(1,length(hisKinThisBoutType1Rand))-1) (zeros(1,length(thisKinThisBoutType2Rand))+1)];
