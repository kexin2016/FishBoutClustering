...
%     indDataSet =  [85:92];
 ...
 %% rot data  not conv - unconv - CS
     indDataSet =  [93 97 101 105 109 114 119];
     
 %% rot data conv - conv
%        indDataSet =  [94 98 102 106 110 115 120];     
     
     
 %% rot data conv - unconv - CS slow?
%       indDataSet =  [95 99 103 107 111 116 121];

 %% rot data conv - stable
%       indDataSet =  [96 100 104 108 112 117 122];
...
%% look at all j turns
indDataSet =  [ 94 98 102 106 110 115 120 96 100 104 108 112 117 122];     
...
%     subplot(1,3,3)
%     plot(dataThisDataSetRand(thisIndRandThis,1),dataThisDataSetRand(thisIndRandThis,3), '.k')
%     axis square
%     box off
%     xlabel('dim 1')
%     ylabel('dim 1')
...
%     axis([-17 22 -10 20])
%     hold on
 sizeOfCluster = zeros(1,length(indChoosenClusterCenters));
    for nn = 1 : length(indChoosenClusterCenters)
       
    indThisCluster = find(clusterAssignment == uniqueClusterAssignment(nn));    
    
    sizeOfCluster(nn) = length(indThisCluster);
...
title(length(indThisCluster))
   
     subplot(2,2,3)
%     plot3(dataThisDataSetRand(indThisCluster,1),dataThisDataSetRand(indThisCluster,2),realRho(indThisCluster), '.', 'color', col(nn,:))
%     plot(dataThisDataSetRand(indThisCluster,1),realRho(indThisCluster), '.', 'color', col(nn,:))
    plot(dataThisDataSetRand(indThisCluster,2),dataThisDataSetRand(indThisCluster,3), '.', 'color', col(nn,:))

    
    hold on
    
    %     plot(dataThisDataSetRand(indChoosenClusterCenters(nn),1),realRho(indChoosenClusterCenters(nn)),'o','MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor',col(nn,:))
    plot(dataThisDataSetRand(indChoosenClusterCenters(nn),2),dataThisDataSetRand(indChoosenClusterCenters(nn),3),'o','MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor',col(nn,:))
...
%     plot3(dataThisDataSetRand(indThisCluster,1),dataThisDataSetRand(indThisCluster,2),realRho(indThisCluster), '.', 'color', col(nn,:))
%     plot(dataThisDataSetRand(indThisCluster,1),realRho(indThisCluster), '.', 'color', col(nn,:))
    plot(dataThisDataSetRand(indThisCluster,2),dataThisDataSetRand(indThisCluster,4), '.', 'color', col(nn,:))
    ...
    %     plot(dataThisDataSetRand(indChoosenClusterCenters(nn),1),realRho(indChoosenClusterCenters(nn)),'o','MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor',col(nn,:))
    plot(dataThisDataSetRand(indChoosenClusterCenters(nn),2),dataThisDataSetRand(indChoosenClusterCenters(nn),4),'o','MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor',col(nn,:))
...
thisInds = indStructure(113).thisInds;
unique(BoutInf(thisInds,EnumeratorBoutInf.dataSetNumber))
sizeOfCluster
%%
