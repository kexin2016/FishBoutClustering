...
%     indDataSet =  [85:92];
 ...
 %% rot data  not conv - unconv - CS
      indDataSet =  [93 97 101 105 109 114];
     
 %% rot data conv - conv
%     indDataSet =  [94 98 102 106 110 115];     
     
     
 %% rot data conv - unconv - CS slow?
%       indDataSet =  [95 99 103 107 111 116];

 %% rot data conv - stable
%       indDataSet =  [96 100 104 108 112 117];
...
%     plot3(dataThisDataSetRand(indThisCluster,1),dataThisDataSetRand(indThisCluster,2),realRho(indThisCluster), '.', 'color', col(nn,:))
%     plot(dataThisDataSetRand(indThisCluster,1),realRho(indThisCluster), '.', 'color', col(nn,:))
    plot(dataThisDataSetRand(indThisCluster,1),dataThisDataSetRand(indThisCluster,3), '.', 'color', col(nn,:))

    
    hold on
    
    %     plot(dataThisDataSetRand(indChoosenClusterCenters(nn),1),realRho(indChoosenClusterCenters(nn)),'o','MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor',col(nn,:))
    plot(dataThisDataSetRand(indChoosenClusterCenters(nn),1),dataThisDataSetRand(indChoosenClusterCenters(nn),3),'o','MarkerSize',10,'MarkerEdgeColor','k','MarkerFaceColor',col(nn,:))
